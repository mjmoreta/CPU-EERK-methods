...
for ll=1:9
    ...
    for kk=1:n
        ...
        % Boundary Chu(tn+5k6)
        MM1=cos(t+k56+y(1))+cos(t+k56+x(1));
        MM2=cos(t+k56+x(2))+cos(t+k56+y(2))+cos(t+k56);
        Chtilde(1,1)=bb*MM1+cc*MM2;
        for ii=2:dim-1
            MM1=cos(t+k56+x(ii));
            MM2=cos(t+k56+x(ii+1))+cos(t+k56+x(ii-1));
            Chtilde(ii,1)=bb*MM1+cc*MM2;
        end
        MM1=cos(t+k56+1+y(1))+cos(t+k56+x(dim));
        MM2=cos(t+k56+1+y(2))+cos(t+k56+1)+cos(t+k56+x(dim-1));
        Chtilde(dim,1)=bb*MM1+cc*MM2;           
        for m=2:dim-1
            MM1=cos(t+k56+y(m));
            MM2=cos(t+k56+y(m+1))+cos(t+k56+y(m-1));
            Chtilde(1+(m-1)*dim,1)=bb*MM1+cc*MM2;
            
            MM1=cos(t+k56+1+y(m));
            MM2=cos(t+k56+1+y(m+1))+cos(t+k56+1+y(m-1));
            Chtilde(dim+(m-1)*dim,1)=bb*MM1+cc*MM2;
        end           
        MM1=cos(t+k56+x(1)+1)+cos(t+k56+y(dim));
        MM2=cos(t+k56+x(2)+1)+cos(t+k56+1)+cos(t+k56+y(dim-1));
        Chtilde(1+(dim-1)*dim,1)=bb*MM1+cc*MM2;
        for ii=2:dim-1
            MM1=cos(t+k56+x(ii)+1);
            MM2=cos(t+k56+x(ii+1)+1)+cos(t+k56+x(ii-1)+1);
            Chtilde(ii+(dim-1)*dim,1)=bb*MM1+cc*MM2;
        end
        MM1=cos(t+k56+1+y(dim))+cos(t+k56+x(dim)+1);
        MM2=cos(t+k56+2)+cos(t+k56+1+y(dim-1))+cos(t+k56+x(dim-1)+1);
        Chtilde(dim2,1)=bb*MM1+cc*MM2;
        
        Chuk56=mult*multMm1(dim,Chtilde);                       
        ...
        % Boundary Dh f(t+k56,u(t+k56))-ut(t+k56))         
        Dhtilde(1,1)=2*cos(t+k56+y(1))+2*cos(t+k56+x(1));
        for ii=2:dim-1
            Dhtilde(ii,1)=2*cos(t+k56+x(ii));
        end
        Dhtilde(dim,1)=2*cos(t+k56+1+x(1))+2*cos(t+k56+x(dim));        
        for m=2:dim-1
            Dhtilde(1+(m-1)*dim,1)=2*cos(t+k56+y(m));
            Dhtilde(dim+(m-1)*dim,1)=2*cos(t+k56+1+y(m));            
        end        
        Dhtilde(1+(dim-1)*dim,1)=2*cos(t+k56+y(dim))+2*cos(t+k56+x(1)+1);
        for ii=2:dim-1
            Dhtilde(ii+(dim-1)*dim,1)=2*cos(t+k56+x(ii)+1);            
        end
        Dhtilde(dim2,1)=2*cos(t+k56+1+y(dim))+2*cos(t+k56+x(dim)+1);

        Dhfuk56=multMm1(dim,Dhtilde);
        ...
        G4=K4.^2+vechk3+Chuk3+Dhfuk3-G1;              
        vecb4(:,1)=U;
        vecb4(:,2)=G1;
        vecb4(:,3)=(-4*G3+9*G4)/k;
        vecb4(:,4)=(24*G3-36*G4)/k^2;          
        K5=phipmM9points(k56,JJ,vecb4,10^(-13),1,1);  
                        
        tie(1)=k3;
        tie(2)=k56;        
        Uaux=phipm_simul_iom9points(tie,JJ,vecb4,10^(-13),1,2);
        K6=Uaux(:,1);
        K5=Uaux(:,2);
        
        % % Approximation to the exact solution at time t_{n+1} 
        G5=K5.^2+vechk56+Chuk56+Dhfuk56-G1;  
        G6=K6.^2+vechk3+Chuk3+Dhfuk3-G1; 
        vecb4(:,3)=(5*G6-(4/5)*G5)/k;
        vecb4(:,4)=(-12*G6+(24/5)*G5)/k^2;        
        U=phipmM9points(k,JJ,vecb4,10^(-13),1,1);
